...
def _generate_sql_statements(**context):
    """
    Generates SQL statements based on new entries and writes them to a file.

    Args:
        context (dict): The task context dictionary containing information for task execution.
    """
    entries = context["ti"].xcom_pull(
        task_ids="process_updates", key="new_entries_clean"
    )
    sql_statements = ""
...
def _generate_telegram_message(**context):
    """
    Generates a Telegram message with links to new articles and pushes it to XCom.
...
with DAG(
    ...
    fetch_feed_updates = BranchPythonOperator(
        task_id="check_feed_updates", python_callable=fetch_feed_updates
    )

    stop_dag_no_updates = DummyOperator(task_id="stop_dag_no_updates")
    ...
    stop_dag_no_articles = DummyOperator(task_id="stop_dag_no_articles")
